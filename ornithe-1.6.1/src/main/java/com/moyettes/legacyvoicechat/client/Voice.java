...
public class Voice implements ClientModInitializer {
	...
	// 1.6-era OSL widened CustomPayload to DataInput/DataOutput (interface
	// parents). common's payloads expose DataInputStream/DataOutputStream
	// overloads only, so route through a stream wrapper for each call.
	public static final class OslModSupported extends VoiceModSupportedPayload
		...
		@Override
		public void read(java.io.DataInput input) throws IOException {
			super.read(input instanceof java.io.DataInputStream
				? (java.io.DataInputStream) input
				: new java.io.DataInputStream(new java.io.ByteArrayInputStream(readAll(input))));
		}
	...
	private static byte[] readAll(java.io.DataInput in) throws IOException {
		java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream();
		try {
			while (true) baos.write(in.readByte());
